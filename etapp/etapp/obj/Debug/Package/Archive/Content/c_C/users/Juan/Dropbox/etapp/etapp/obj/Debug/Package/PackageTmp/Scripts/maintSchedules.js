...
function getMaintSchedules_v2(deviceId, taskId) {
    try {
        ...
        var data = 'deviceId=' + escape(deviceId) + '&taskId=' + taskId;
        $.ajax({
            type: "GET",
            url: 'etrest.svc/getMaintSchedules/' + escape(token),
...
function btnSchedApplyFilter() {
    try {
        var devId = $('#cbxSchedDevFilter').val();
        var taskId = $('#cbxSchedTaskFilter').val();
