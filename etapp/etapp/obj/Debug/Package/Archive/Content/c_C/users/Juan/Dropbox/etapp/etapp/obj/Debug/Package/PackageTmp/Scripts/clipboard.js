(function () {
    ...
    function normalizeURI(href) {
        ...
        if (!href) return "";
        if (href.substring(0, 2) === "//") href = "http://" + href.substring(2);
        ...
        else if (!(prefixedBy(href, "http://") || prefixedBy(href, "https://") || prefixedBy(href, "ftp://") || prefixedBy(href, "file://") || prefixedBy(href, "data:"))) {
    ...
    function doDomainSpecificHacks(clip) {
        ...
        function ytEmbedString(id, width, height) {
            return '<iframe src="http://www.youtube.com/embed/{id}?wmode=transparent" width="{w}" height="{h}" frameborder="0" allowfullscreen></iframe>'.replace("{id}", id).replace("{w}", width).replace("{h}", height)
        }
        var embed;
