...
function loadReports() {
    try {
        ...
        if (jsonReports) {
            ...
            loadComboBox(jsonReports.reports, cbx, 'Pick a report');
        }
    }
    catch (err) {
...
function saveRecRep() {
    try {
        ...
        data = 't=' + getTokenCookie('ETTK') + '&id=' + escape(id) + '&reportId=' + escape(reportId) + '&frequencyId=' + escape(frequencyId) + '&excludeWeekends=' + escape(excludeWeekends) + '&isAllDevices=' + escape(isAllDevices) + '&devices=' + escape(jsonDevicesTXT) + '&isAllUsers=' + escape(isAllUsers) + '&users=' + escape(jsonUsersTXT);
        var tmpJson = dbReadWrite('saveRecurrentReport', data, true, false);

        if (tmpJson.result != 'failure') {
            var itm = { 'id': tmpJson.result, 'reportId': reportId, 'reportName': reportName, 'frequencyId': frequencyId, 'frequencyName': frequencyName, 'excludeWeekends': excludeWeekends, 'isAllDevices': isAllDevices, 'createdOn': 'Today' };
            if (id == '0' || id == '') {
                addRecRepToList(itm);
            }
            else {
