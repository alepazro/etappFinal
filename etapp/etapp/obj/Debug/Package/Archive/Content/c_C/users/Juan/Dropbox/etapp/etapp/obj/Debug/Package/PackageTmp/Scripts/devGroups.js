...
function saveDevGroup() {
    try {
        ...
        data = 't=' + getTokenCookie('ETTK') + '&id=' + escape(id) + '&name=' + escape(name) + '&isAllDevices=' + escape(isAllDevices) + '&devices=' + escape(jsonDevicesTXT) + '&isAllUsers=' + escape(isAllUsers) + '&users=' + escape(jsonUsersTXT);
        var tmpJson = dbReadWrite('saveDeviceGroup', data, true, false);

        if (tmpJson.result != 'failure') {
            var itm = { 'id': tmpJson.result, 'name': name, 'isPublic': true, 'isAllDevices': isAllDevices, 'isAllUsers':isAllUsers, 'isDefault': false, 'createdOn': 'Today' };
            if (id == '0' || id == '') {
